...
| GET | `/admin/feedback` | 管理パスワードで保護したアンケート集計を取得 |
...
管理者はWeb画面の `/admin` で、回答の件数・割合・モード別集計・検索ごとの入力と推薦結果を確認できます。管理画面のAPIは `ADMIN_DASHBOARD_TOKEN` と一致するBearerトークンがない限り内容を返しません。
