...
python -m src.metadata_fallback blend \
  ...
  --output-dir reports/serving_metadata_evidence_v2 \
  --max-broad-metadata-only-top10 3 \
...
人手評価で見つかった単独方式の弱さには、アーティスト名ごとの例外ではなく全候補共通の証拠条件を使います。pop・rock・J-popなど広いジャンルだけの候補は、同じ国・同じ種別・活動開始10年以内もそろった場合だけ採用し、Top 10内で最大3件に制限します。聴取データだけの候補は共通リスナー3人以上を必須とし、3人未満なら強いメタデータ証拠との組合せが必要です。条件を通る候補が10件未満なら、弱い候補で埋めず件数を減らします。Wikidataは100 MBIDずつ分割取得してキャッシュし、MusicBrainzの個別照会は低データ入力に絞ります。
...
共通ルール適用後は8組の上位候補が80件から76件になり、前回評価0の14件中13件が外れました。48件は前回評価を引き継げますが、新規28件はまだ人手評価前です。7組はTop 10を生成し、ヤングスキニーは条件を通った6件だけを返します。これは改善の途中結果であり、新規28件を採点するまでは新モデルを有効化しません。
...
共通の証拠条件を入れた `cosine-shrinkage-evidence-rules-30d-v3` は4,456辺・根拠4,545件を非アクティブで追加保存しました。現在の有効モデルは引き続きv2の4,500辺で、新規28件の人手評価が終わるまで切り替えません。旧モデルもロールバック用に残るため、ローカルSQLite全体は1,978アーティスト・18,816辺・根拠13,645件です。DBファイルは生成物としてGit管理しません。
