...
def similarity_output_name(limit: int) -> str:
    if limit <= 0:
        raise ValueError("limit must be positive")
    return f"similarity_top{limit}.csv"
        ...
        for record_batch in dataset.to_batches(columns=columns, batch_size=65_536):
            ...
            for user_id, artist_name, mbids in zip(
                ...
                if len(pending) >= batch_size:
                    ...
                    connection.commit()
                    pending.clear()
            if progress_interval > 0 and rows_seen // progress_interval != (
                rows_seen - record_batch.num_rows
            ) // progress_interval:
                print(f"Aggregate: {rows_seen:,} listen rows", flush=True)
...
def load_targets(
    coverage_path: Path, category: str | None = None
) -> dict[str, dict[str, str]]:
    with coverage_path.open(encoding="utf-8", newline="") as handle:
        rows = list(csv.DictReader(handle))
    targets: dict[str, dict[str, str]] = {}
    for row in rows:
        if category is not None and row.get("category", "").strip() != category:
            continue
...
def load_spark_manifest(path: Path) -> list[Path]:
    manifest = json.loads(path.read_text(encoding="utf-8"))
    sources = manifest.get("sources")
    if not isinstance(sources, list) or not sources:
        raise ValueError(f"No sources in Spark manifest: {path}")
    result = []
    for item in sources:
        if not isinstance(item, dict) or not str(item.get("path", "")).strip():
            raise ValueError(f"Invalid source in Spark manifest: {item!r}")
        result.append(Path(str(item["path"])))
    return result
...
def summarize_similarity_rows(
    ...
    return {
        ...
        "target_artists_with_listeners": sum(
            listener_counts.get(mbid, 0) > 0 for mbid in targets
        ),
        ...
        "expected_top10_hit_rate_with_recommendations": (
            round(expected_hits_available / evaluated_available, 4)
...
def compute_similarities_sequential(
    database: Path,
    targets: dict[str, dict[str, str]],
    config: SimilarityConfig,
) -> tuple[list[dict[str, Any]], dict[str, Any]]:
    """Compute the same cosine+shrinkage score one seed at a time to cap memory use."""
    connection = sqlite3.connect(database)
    target_mbids = set(targets)

    norms_squared: dict[str, float] = defaultdict(float)
    listener_counts: dict[str, int] = defaultdict(int)
    for mbid, listen_count in connection.execute(
        "SELECT artist_mbid, listen_count FROM user_artist"
    ):
    ...
    """
    for mbid, name, _total in connection.execute(name_query):
        names.setdefault(mbid, name)

    rows: list[dict[str, Any]] = []
    pair_query = """
        SELECT seed.listen_count, candidate.artist_mbid, candidate.listen_count
        FROM user_artist AS seed
        JOIN user_artist AS candidate ON candidate.user_key = seed.user_key
        WHERE seed.artist_mbid = ?
    """
    for seed_number, (seed_mbid, target) in enumerate(targets.items(), 1):
        dots: dict[str, float] = defaultdict(float)
        common: dict[str, int] = defaultdict(int)
        for seed_count, candidate_mbid, candidate_count in connection.execute(
            pair_query, (seed_mbid,)
        ):
            if candidate_mbid == seed_mbid:
                continue
            dots[candidate_mbid] += weight(seed_count, config.listen_cap) * weight(
                candidate_count, config.listen_cap
            )
            common[candidate_mbid] += 1

        candidates: list[dict[str, Any]] = []
        for candidate_mbid, dot in dots.items():
            common_count = common[candidate_mbid]
            if common_count < config.min_common_listeners:
                continue
            denominator = math.sqrt(
                norms_squared[seed_mbid] * norms_squared[candidate_mbid]
            )
            if denominator <= 0:
                continue
            candidate_name = (
                targets.get(candidate_mbid, {}).get("artist_name")
                or names.get(candidate_mbid, "")
            )
            if not candidate_name or not valid_candidate_name(candidate_name):
                continue
            ...
            candidates.append(
                {
                    ...
                    "similarity_score": round(cosine * shrinkage_factor, 6),
        ...
        for rank, row in enumerate(candidates[: config.limit], 1):
            row["rank"] = rank
            rows.append(row)
        print(
            f"Similarity: {seed_number}/{len(targets)} {target.get('artist_name', '')}",
            flush=True,
        )

    placeholders = ",".join("?" for _ in target_mbids)
    selected_users = connection.execute(
        f"SELECT COUNT(DISTINCT user_key) FROM user_artist "
        f"WHERE artist_mbid IN ({placeholders})",
        tuple(target_mbids),
    ).fetchone()[0]
    connection.close()
    rows.sort(key=lambda row: (row["seed_artist_name"].casefold(), row["rank"]))
    return rows, summarize_similarity_rows(
        targets, rows, listener_counts, selected_users, config
    )
...
def valid_candidate_name(name: str) -> bool:
    """Exclude aggregate or placeholder credits that are not recommendable artists."""
    return normalize_name(name) not in {"unknown", "unknownartist", "variousartists"}
...
def write_summary(path: Path, dump_stats: dict[str, int], summary: dict[str, Any]) -> None:
    ...
    lines = [
        ...
        f"- {summary['target_artist_count']}組のうち期間内リスナーあり: {summary['target_artists_with_listeners']}/{summary['target_artist_count']}",
        f"- {summary['target_artist_count']}組のうち推薦を出せた数: {summary['target_artists_with_recommendations']}/{summary['target_artist_count']}",
        f"- 手入力した期待候補のTop {top_k} hit率（全{summary['target_artist_count']}組）: {summary['expected_top10_hit_rate_all_targets']:.1%}",
...
def run(
    ...
    # The target-user batch implementation is fast for small samples but can
    # retain many candidate pairs in memory. Production-sized windows use the
    # indexed, seed-by-seed implementation to keep memory bounded.
    rows, summary = compute_similarities_sequential(work_db, targets, config)
    report_dir.mkdir(parents=True, exist_ok=True)
    write_csv(report_dir / similarity_output_name(config.limit), rows)
...
def run_from_database(
    ...
    """Recompute target similarities without rebuilding the aggregate database."""
    if not work_db.exists():
        raise FileNotFoundError(f"Aggregate database not found: {work_db}")
    previous = json.loads(dump_stats_json.read_text(encoding="utf-8"))
    dump_stats = previous.get("dump")
    if not isinstance(dump_stats, dict):
        raise ValueError(f"Missing dump statistics in {dump_stats_json}")
    targets = load_targets(coverage, category)
    rows, summary = compute_similarities_sequential(work_db, targets, config)
    report_dir.mkdir(parents=True, exist_ok=True)
    write_csv(report_dir / similarity_output_name(config.limit), rows)
    result = {"dump": dump_stats, "similarity": summary}
...
def build_parser() -> argparse.ArgumentParser:
    ...
    source.add_argument(
        "--spark-manifest",
        type=Path,
        help="Manifest JSON created by src.listenbrainz_window",
    )
    source.add_argument("--archive", type=Path, help="Raw ListenBrainz listens .tar.zst (lower coverage)")
    source.add_argument(
        "--reuse-work-db",
        action="store_true",
        help="Reuse --work-db and only recompute similarities for the current coverage targets",
    )
    ...
    parser.add_argument(
        "--dump-stats-json",
        type=Path,
        default=DEFAULT_REPORT_DIR / "similarity_summary.json",
        help="Previous similarity JSON containing dump statistics when --reuse-work-db is used",
    )
    ...
    parser.add_argument(
        "--category",
        help="Only include coverage rows with this category (supported with --reuse-work-db)",
    )
    return parser
...
def main() -> None:
    ...
    if args.spark_manifest is not None:
        spark_archive = load_spark_manifest(args.spark_manifest)

    if args.reuse_work_db:
        _, result = run_from_database(
            args.coverage,
            args.report_dir,
            args.work_db,
            config,
            args.dump_stats_json,
            args.category,
        )
    else:
        _, result = run(
